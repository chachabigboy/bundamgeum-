export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Mode B: 법정동코드로 단지 목록 → kaptCode 자동 검색 ──
    if (!bjdCode) return res.status(400).json({ error: 'bjdCode 또는 kaptCode 필요' });

    // AptListService: 법정동코드로 단지 목록 + kaptCode 조회
    const listUrl =
      `https://apis.data.go.kr/1611000/AptListService2/getLegaldongAptList` +
      `?serviceKey=${KEY}&loadCode=${bjdCode}&_type=json&numOfRows=100&pageNo=1`;
...
function extractTypes(item) {
  if (!item) return null;
  const types = [];
  // kaptMparea60, kaptMparea85, kaptMparea135 등 평형별 세대수 필드
  ...
  // privArea 필드 파싱 (쉼표 구분 면적 목록)
  if (!types.length && item.privArea) {
    item.privArea.split(',').forEach(s => {
      const area = parseFloat(s.trim());
      if (area > 0) types.push({ dedicArea: area, hhldCnt: 0, pyeong: Math.round(area / 3.3058) });
    });
  }
